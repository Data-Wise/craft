...
> Reference material surfaced by `skills/dev/git/SKILL.md` (Operation 11)
> when the user says "teach me git" or wants the progressive learning path.
> Moved here from `commands/git/docs/learning-guide.md` (now a thin shim)
> as part of the deprecated-command audit (issue #233). Command names
> referenced below (`/done`, `/commit`, `/git-recap`, `/sync`, `/branch`,
> `/pr-review`, `/pr-create`) are illustrative shorthand for the underlying
> craft git-workflow operations, not necessarily exact current slash-command
> names — see `skills/dev/git/SKILL.md`'s Integration table for the real
> `/craft:git:*` mapping.
