...
def test_done_command_is_shim_routing_to_skill():
    """commands/finish.md (renamed from done.md, ADR-006) must be a thin shim
    that routes to the canonical skill reference (ADR-002), not a duplicate
    of the full body."""
    shim = open(os.path.join(CRAFT, "commands", "finish.md"), encoding="utf-8").read()
