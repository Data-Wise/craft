...
When an agent fails, times out, or needs retry/escalation handling, load the
`orchestrator-resilience` skill for the full reference: error categorization,
exponential backoff schedule, timeout handling, fallback agent selection,
circuit breaker pattern, error response protocol, escalation hierarchy,
mode-specific recovery thresholds, error aggregation, and self-healing.

Quick summary (full templates in the skill):

- Classify errors as **transient**, **resource**, **configuration**, **logical**,
  or **permanent** — this determines retry vs. escalate.
- Retry transient/resource errors with exponential backoff (2s → 4s → 8s → 16s max).
- After 3 consecutive failures on one agent, treat its circuit as OPEN — skip it,
  use a fallback, or escalate rather than continuing to retry.
- Escalate to the user (via `AskUserQuestion`, per BEHAVIOR 6) when a failure
  requires a design decision, not just a retry.
- Recovery thresholds vary by mode — `debug` escalates on any error, `release`
  only on critical ones (see skill for the full per-mode table).
