...
if [ -f "docs/REFCARD.md" ]; then
    if [ "$DRY_RUN" = true ]; then
        ...
        sed -i '' "s|^## Skills ([0-9][0-9]* total)|## Skills (${SKILL_COUNT} total)|" docs/REFCARD.md
        sed -i '' "s|^## Agents ([0-9][0-9]* specialized)|## Agents (${AGENT_COUNT} specialized)|" docs/REFCARD.md
...
if [ -f "docs/skills-agents.md" ]; then
    if [ "$DRY_RUN" = true ]; then
        ...
        sed -i '' "s|^### Architecture ([0-9][0-9]*)|### Architecture (${SKILL_ARCH})|"   docs/skills-agents.md
        sed -i '' "s|^### Check ([0-9][0-9]*)|### Check (${SKILL_CHECK})|"                 docs/skills-agents.md
        sed -i '' "s|^### CI ([0-9][0-9]*)|### CI (${SKILL_CI})|"                          docs/skills-agents.md
        sed -i '' "s|^### Code ([0-9][0-9]*)|### Code (${SKILL_CODE})|"                     docs/skills-agents.md
        sed -i '' "s|^### Design ([0-9][0-9]*)|### Design (${SKILL_DESIGN})|"               docs/skills-agents.md
        sed -i '' "s|^### Dev ([0-9][0-9]*)|### Dev (${SKILL_DEV})|"                        docs/skills-agents.md
        sed -i '' "s|^### Distribution ([0-9][0-9]*)|### Distribution (${SKILL_DIST})|"     docs/skills-agents.md
        sed -i '' "s|^### Documentation ([0-9][0-9]*)|### Documentation (${SKILL_DOCS})|"   docs/skills-agents.md
        sed -i '' "s|^### Guard & Insights ([0-9][0-9]*)|### Guard \& Insights (${SKILL_GUARD})|" docs/skills-agents.md
        sed -i '' "s|^### Modes ([0-9][0-9]*)|### Modes (${SKILL_MODES})|"                  docs/skills-agents.md
        sed -i '' "s|^### Orchestration ([0-9][0-9]*)|### Orchestration (${SKILL_ORCH})|"   docs/skills-agents.md
        sed -i '' "s|^### Planning ([0-9][0-9]*)|### Planning (${SKILL_PLAN})|"             docs/skills-agents.md
        sed -i '' "s|^### Release ([0-9][0-9]*)|### Release (${SKILL_RELEASE})|"            docs/skills-agents.md
        sed -i '' "s|^### Testing ([0-9][0-9]*)|### Testing (${SKILL_TEST})|"               docs/skills-agents.md
        sed -i '' "s|^### Workflow ([0-9][0-9]*)|### Workflow (${SKILL_WORKFLOW})|"         docs/skills-agents.md
        # Agent sub-category headers (item #7)
        sed -i '' "s|^### Documentation Agents ([0-9][0-9]*)|### Documentation Agents (${AGENT_DOCS})|" docs/skills-agents.md
        sed -i '' "s|^### Orchestration Agents ([0-9][0-9]*)|### Orchestration Agents (${AGENT_ROOT})|" docs/skills-agents.md
...
for hub_file in commands/hub.md docs/commands/hub.md; do
    if [ -f "$hub_file" ]; then
        if [ "$DRY_RUN" = true ]; then
            ...
            sed -i '' "s|CODE ([0-9][0-9]*)|CODE (${CMD_CODE})|g"               "$hub_file"
            sed -i '' "s|TEST ([0-9][0-9]*)|TEST (${CMD_TEST})|g"               "$hub_file"
            sed -i '' "s|ARCH ([0-9][0-9]*)|ARCH (${CMD_ARCH})|g"               "$hub_file"
            sed -i '' "s|DOCS ([0-9][0-9]*)|DOCS (${CMD_DOCS})|g"               "$hub_file"
            sed -i '' "s|PLAN ([0-9][0-9]*)|PLAN (${CMD_PLAN})|g"               "$hub_file"
            sed -i '' "s|CI ([0-9][0-9]*)|CI (${CMD_CI})|g"                     "$hub_file"
            sed -i '' "s|GIT ([0-9][0-9]* incl\. [0-9]* guides)|GIT (${CMD_GIT} incl. 4 guides)|g" "$hub_file"
            sed -i '' "s|WORKFLOW ([0-9][0-9]*)|WORKFLOW (${CMD_WORKFLOW})|g"   "$hub_file"
            sed -i '' "s|DIST ([0-9][0-9]*)|DIST (${CMD_DIST})|g"               "$hub_file"
            sed -i '' "s|SITE ([0-9][0-9]*)|SITE (${CMD_SITE})|g"               "$hub_file"
            sed -i '' "s|ORCHESTRATE ([0-9][0-9]*)|ORCHESTRATE (${CMD_ORCH})|g" "$hub_file"
            # Categorical section headers "LABEL COMMANDS (N) - ..."
            sed -i '' "s|CODE COMMANDS ([0-9][0-9]*)|CODE COMMANDS (${CMD_CODE})|g"               "$hub_file"
            sed -i '' "s|TEST COMMANDS ([0-9][0-9]*)|TEST COMMANDS (${CMD_TEST})|g"               "$hub_file"
            sed -i '' "s|DOCS COMMANDS ([0-9][0-9]*)|DOCS COMMANDS (${CMD_DOCS})|g"               "$hub_file"
            sed -i '' "s|WORKFLOW COMMANDS ([0-9][0-9]*)|WORKFLOW COMMANDS (${CMD_WORKFLOW})|g"   "$hub_file"
            sed -i '' "s|SITE COMMANDS ([0-9][0-9]*)|SITE COMMANDS (${CMD_SITE})|g"               "$hub_file"
            sed -i '' "s|ARCH COMMANDS ([0-9][0-9]*)|ARCH COMMANDS (${CMD_ARCH})|g"               "$hub_file"
            sed -i '' "s|CI COMMANDS ([0-9][0-9]*)|CI COMMANDS (${CMD_CI})|g"                     "$hub_file"
            sed -i '' "s|DIST COMMANDS ([0-9][0-9]*)|DIST COMMANDS (${CMD_DIST})|g"               "$hub_file"
            sed -i '' "s|PLAN COMMANDS ([0-9][0-9]*)|PLAN COMMANDS (${CMD_PLAN})|g"               "$hub_file"
            sed -i '' "s|ORCHESTRATE COMMANDS ([0-9][0-9]*)|ORCHESTRATE COMMANDS (${CMD_ORCH})|g" "$hub_file"
