...
The check extracts the command count from the formula's `desc` string (e.g., `"Full-stack developer toolkit for Claude Code with 111 commands"`) and compares it against the actual count detected by `validate-counts.sh`. If they differ, a warning is raised.
