...
|---|---|---|---|
| **1 — Do first** ✅ **DONE 2026-07-04** | D3, D2, D1a | Cheap, independently unblocked, no cross-item dependency. Highest confidence, lowest risk — start here. | Full suite green (2713 passed, 0 failed); `test_scaffold_defaults_e2e.py:39` needed one additional fix beyond the 3 files anticipated — its `test_logic_lives_in_skills_not_deprecated_commands` assumed `feature.md` was still a deprecated shim pointing to `plan-orchestrator`; excluded it from that check since D2 confirmed it's genuinely independent content, never a real shim. Counts unaffected (116/45/8). |
| **2 — Cleanup** ✅ **DONE 2026-07-04** | D4, D6, D9, D10 | Independent of each other and of Wave 1 — pure consolidation/dedupe, no new routing behavior. Can run in any order, even in parallel across sub-agents. | New `verify-gate-detection.md` (D4) referenced correctly from all 3 sites; `--refine` default rule (D6) documented once, not repeated. Executed as: D4+D10 via 2 parallel background `Agent` calls (disjoint files); D6+D9 done directly, sequentially, since both touch `commands/do.md`/`commands/orchestrate.md` (avoided same-file parallel-edit race). Full suite green (2713 passed, 0 failed) both per-item and combined; counts unchanged (116/45/8). One agent (D10) reported completion before its own background verification actually finished — caught by re-running verification independently rather than trusting the self-report (per this session's own `background-agent-monitoring-protocol` practice). |
| **3 — Depends on Waves 1+2** ✅ **DONE 2026-07-04** | D5, D1b | D5 needs Wave 2's dedupe settled (it collapses `do.md`↔`task-analyzer`, which D9's mode-enum dedupe also touches). D1b needs D1a's stubs shrunk first (Wave 1) and benefits from D5/D6 being settled (avoids building the router against tables about to move). | D5: `task-analyzer/SKILL.md`'s Intent Recognition table renamed to `do.md`'s exact 7 categories, folding "Review" into "Architecture" with a documented rationale; `do.md`'s Routing Logic tree removed as fully redundant. D1b: file-vs-alias resolved as **standalone `commands/plan.md`** coexisting with `commands/plan/` (precedent: `commands/orchestrate.md` + `commands/orchestrate/`); added the `--plan` sugar flag to `do.md` per SPEC-planning-refactor-2026-06-22.md §4 rule 4. New command triggered a ~30-file count cascade (116→117 across `plugin.json`, README.md, docs-site, REFCARD.md, mkdocs.yml nav, `test_refine_flag_scope`) plus 2 doc-coverage errors from un-deprecating `plan/feature.md` (D2) newly exposing it to REFCARD/nav coverage checks — all fixed via `bump-version.sh --counts-only` + `docs-staleness-check.sh --fix` + manual REFCARD/nav additions. Full suite green (2715 passed, 0 failed); `docs-staleness-check.sh` GREEN except 1 pre-existing warn-only finding (no tutorial for `plan/feature.md`, non-blocking per script's own severity). |
