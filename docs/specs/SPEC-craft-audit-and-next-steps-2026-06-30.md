...
- WARNs (not blocks, per ADR-003's advisory-not-hard-gate precedent) above a configurable ratio threshold (suggest starting at 2.0 — would currently flag 18 of 56 commands, verified via `python3 scripts/audit-deprecated-commands.py --threshold 2.0`; matches `SPEC-token-efficiency-research-2026-06-30.md` §6, which previously disagreed with this file's earlier "~20" estimate).
