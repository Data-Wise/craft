...
3. Extend the '## See Also' section prose in commands/orchestrate/workflow.md to add two cookbook recipe bullet lines: '- Run a coded workflow — cookbook: write a WORKFLOW-*.yaml, dry-run preview, execute, resume' and '- Fan a workflow across files — cookbook: list → parallel-per-file → gate → summarize pattern'
