...
[![Version](https://img.shields.io/badge/version-2.61.1-brightgreen.svg)](https://github.com/Data-Wise/craft/releases)
...
!!! info "Latest: v2.61.1 — Token-Usage Reduction & Post-Release Follow-ups"
    [PR #232](https://github.com/Data-Wise/craft/pull/232) cuts the always-loaded orchestration path (`/refine` 631→42 lines, `/brainstorm` split with 4→2 decision points, `orchestrator-v2.md` 1473→1212 lines) by moving procedure out of commands/agents and into skills that load conditionally — plus model-pins the orchestrator agents (`sonnet`/`haiku`) for the first time. New skill: [`command-skill-token-efficiency`](skills-agents.md#code-4) codifies the methodology for future authoring.
    **Post-release follow-ups:** [PR #270](https://github.com/Data-Wise/craft/pull/270) removes dead agent-dispatch from `/craft:do` and `/release`, adds `verify-surfaces.sh --report-only`/`--version` for never-blocking drift inspection, and ships a [`release-rolldown`](runbooks/release-rollback.md) manual-undo runbook. [PR #272](https://github.com/Data-Wise/craft/pull/272) consolidates `guard.md`/`protect.md` into `skills/dev/git/`, thins 4 stale shims, and adds a unified `--classify`/`GUARD_DRY_RUN=1` ground-truth mode across both guard hooks.
    See the [full changelog](CHANGELOG.md) for all releases, including the prior v2.43.0 skill-ecosystem governance work (`governance/` policy-as-code, see the [governance guide](guide/governance.md)).
