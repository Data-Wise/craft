...
7. **Pre-commit hooks** - Auto-validate docs before commit
8. **Initial commit** - Commit setup with conventional message
9. **Push to GitHub** - Sync branches to remote
10. **Validate setup** - Run /craft:check
...
```
AskUserQuestion: "Enable pre-commit hooks for documentation quality?"
  ○ Yes (Recommended) - Auto-validate docs before commit
  ○ No - Skip hooks (can enable later)
```

If yes, creates `.git/hooks/pre-commit`:

```bash
#!/bin/bash
# Auto-generated by /craft:git:init
...
if [ -n "$STAGED_MD" ]; then
  ...
  # Step 1: Markdown linting with auto-fix
  echo "  → Running markdown lint..."
  if ! claude "/craft:docs:lint --fix"; then
    echo "❌ Markdown linting found issues requiring manual fix"
    echo "   Review the issues above and fix them, then try again."
    exit 1
  fi

  # Step 2: Link validation
  echo "  → Checking links..."
  if ! claude "/craft:docs:check-links default"; then
...
│                                                             │
│ ✓ Pre-commit Hooks                                          │
│   - .git/hooks/pre-commit (docs quality checks)            │
│   - Runs /craft:docs:lint --fix on staged docs             │
│   - Runs /craft:docs:check-links on staged docs            │
│                                                             │
│ ✓ Initial Commit                                            │
...
- **Enable hooks**: Pre-commit hooks prevent broken docs from being committed
- **Run check**: `/craft:check` validates your setup
- **Customize templates**: Edit `.claude/templates/` for your team's standards
- **Skip hooks temporarily**: Use `git commit --no-verify` when needed
...
**Issue:** Hook prevents committing docs with issues

```bash
# View what hook is checking
cat .git/hooks/pre-commit

# See actual issues
/craft:docs:lint docs/
/craft:docs:check-links docs/

# Fix issues and try again
/craft:docs:lint --fix docs/
git add docs/
...
**Issue:** Hook exists but doesn't execute
